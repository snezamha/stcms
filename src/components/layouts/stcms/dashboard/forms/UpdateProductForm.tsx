...
export function UpdateProductForm({ product }: { product: Product }) {
  ...
  const onSubmit = async (values: productPayload) => {
    try {
      setIsLoading(true);
      await axios.patch(
        `/api/stcms/store/${params.storeId}/product/${product.id}`,
        values
      );
      toast({
        title: t('productHasBeenUpdated')
      });
      window.location.assign(`/dashboard/store/${params.storeId}`);
    } catch (error: any) {
      toast({
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        {t('updateProduct')}
          <span className='sr-only'>{t('updateProduct')}</span>
        </Button>
      </form>
    </Form>
  );
